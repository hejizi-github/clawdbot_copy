...
trajeval 支持 OpenTelemetry OTLP JSON 导出格式，遵循 [GenAI 语义约定](https://opentelemetry.io/docs/specs/semconv/gen-ai/)：

```bash
trajeval eval otlp_export.json --input-format otlp
trajeval compare baseline_otlp.json current_otlp.json --input-format otlp
```

OTLP span 到 TraceStep 的映射：

| OTLP span 特征 | trajeval step 类型 |
|---|---|
| `gen_ai.operation.name` = chat/text_completion | `llm_call` |
| `gen_ai.operation.name` = execute_tool | `tool_call` |
| `status.code` = 2 (ERROR) | `error` |
| `gen_ai.operation.name` = invoke_agent | `decision` |

支持的 GenAI 属性：`gen_ai.request.model`、`gen_ai.usage.input_tokens`/`output_tokens`、`gen_ai.tool.name`、`gen_ai.agent.name`、`gen_ai.provider.name` 等。

### Clawdbot JSONL

Clawdbot/OpenClaw 会话日志自动识别（`.jsonl` 后缀），或手动指定 `--input-format clawdbot`。
